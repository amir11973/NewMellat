<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PageEstelamGharardad.aspx.cs" Inherits="NewMellat.Content.PageEstelamGharardad" %>
        ...
        <dx:ASPxGridView ID="ASPxGridView1" runat="server" Theme="SoftOrange" AutoGenerateColumns="False" DataSourceID="sourcePages" KeyFieldName="paId" RightToLeft="True" Width="100%" OnHtmlDataCellPrepared="ASPxGridView1_HtmlDataCellPrepared" OnDataBound="ASPxGridView1_DataBound">
            ...
            <Columns>
                ...
                <dx:GridViewDataTextColumn Caption="پاسخ" VisibleIndex="6">
                    <EditFormSettings Visible="False" />
                    <DataItemTemplate>
                        <asp:HyperLink ID="HyperLink1" runat="server" ImageUrl="~/Content/images/icons/check-icon.png" NavigateUrl='<%# Eval("paId", "~/content/PageAnswergharardad.aspx?id={0}") %>'></asp:HyperLink>
                    </DataItemTemplate>
                </dx:GridViewDataTextColumn>
            </Columns>
            <Styles>
            ...
            <asp:SqlDataSource ID="sourcePages" runat="server" ConnectionString="<%$ ConnectionStrings:NewMellatConnectionString %>" DeleteCommand="UPDATE tbProjectPaper SET paAct = 0 WHERE (paId = @paId)" InsertCommand="INSERT INTO tbProjectPaper(paShenase, paNumber, paDate, paProject, paAct, paType, paBody) VALUES (@paShenase, @paNumber, @paDate, @paProject, 1, @paType, @paBody)" SelectCommand="SELECT paId, paShenase, paNumber, paDate, paProject, paAct, paType, paBody, paEdare FROM tbProjectPaper WHERE (paAct = 1) AND (paType = @paType) AND (paProject = @paProject)" UpdateCommand="UPDATE tbProjectPaper SET paShenase = @paShenase, paNumber = @paNumber, paBody = @paBody, paEdare = @paEdare WHERE (paId = @paId)">
                ...
                <SelectParameters>
                    <asp:QueryStringParameter Name="paType" QueryStringField="type" />
                    <asp:QueryStringParameter DefaultValue="" Name="paProject" QueryStringField="id" />
                </SelectParameters>
                <UpdateParameters>
