...
SELECT @paId = SCOPE_IDENTITY()" SelectCommand="SELECT paId, paShenase, paNumber,  paProject, paAct, paType, paBody,dbo.getShamsiDate(paDate) as paDate,  
paEdare=
case paEdare
when 1 then N'تایید'
when 2 then N'عدم تایید'
end
 FROM tbProjectPaper WHERE (paAct = 1) AND (paType = @paType) AND (paProject = @paProject)" UpdateCommand="UPDATE tbProjectPaper SET paShenase = @paShenase, paNumber = @paNumber, paBody = @paBody, paEdare = @paEdare WHERE (paId = @paId)" OnInserted="sourcePages_Inserted">
                <DeleteParameters>
