<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PageEdarat.aspx.cs" Inherits="NewMellat.Content.PageEdarat" %>
    ...
    <div align="center">
        ...
        <dx:ASPxGridView ID="ASPxGridView1" runat="server" Theme="SoftOrange" AutoGenerateColumns="False" DataSourceID="sourcePages" KeyFieldName="paId" RightToLeft="True" Width="100%" OnHtmlDataCellPrepared="ASPxGridView1_HtmlDataCellPrepared" OnDataBound="ASPxGridView1_DataBound">
            ...
            <Columns>
                ...
                <dx:GridViewDataComboBoxColumn Caption="اداره" FieldName="paEdare" VisibleIndex="1">
                    <PropertiesComboBox DataSourceID="sourceInfo" TextField="infTitle" ValueField="infId">
                    </PropertiesComboBox>
                </dx:GridViewDataComboBoxColumn>
                <dx:GridViewDataTextColumn Caption="پاسخ استعلام" FieldName="nzStatus" VisibleIndex="6">
                    <Settings AutoFilterCondition="Contains" />
                </dx:GridViewDataTextColumn>
                <dx:GridViewDataTextColumn Caption="تاریخ نامه پاسخ شرکت" FieldName="nzDate" VisibleIndex="8">
                    <Settings AutoFilterCondition="Contains" />
                </dx:GridViewDataTextColumn>
                <dx:GridViewDataTextColumn Caption="شناسه نامه پاسخ شرکت" FieldName="nzShenase" VisibleIndex="7">
                    <Settings AutoFilterCondition="Contains" />
                </dx:GridViewDataTextColumn>
            </Columns>
            <Styles>
            ...
            <asp:SqlDataSource ID="sourcePages" runat="server" ConnectionString="<%$ ConnectionStrings:NewMellatConnectionString %>" DeleteCommand="UPDATE tbProjectPaper SET paAct = 0 WHERE (paId = @paId)" InsertCommand="INSERT INTO tbProjectPaper(paShenase, paNumber, paDate, paProject, paAct, paType, paBody, paEdare) VALUES (@paShenase, @paNumber, @paDate, @paProject, 1, @paType, @paBody, @paEdare)" SelectCommand="SELECT tbProjectPaper.paId, tbProjectPaper.paShenase, tbProjectPaper.paNumber, tbProjectPaper.paDate, tbProjectPaper.paProject, tbProjectPaper.paAct, tbProjectPaper.paType, tbProjectPaper.paBody, tbProjectPaper.paEdare , Case ISNull(tbNazar.nzStatus,0) When 195 Then 'OK' When 196 Then 'NotOK' Else '-'End nzStatus
,IsNull(DimDate.PersianStr,'') as nzDate ,IsNull(tbNazar_1.nzShenase,'') nzShenase
FROM tbProjectPaper Left Outer Join tbNazar on  tbProjectPaper.paId = tbNazar.nzParent Left Outer Join
...
DimDate On tbNazar_1.nzDate = DimDate.GregorianDate
WHERE --(tbProjectPaper.paAct = 1) AND 
                (tbProjectPaper.paType = @paType) AND (tbProjectPaper.paProject = @paProject)
" UpdateCommand="UPDATE tbProjectPaper SET paShenase = @paShenase, paNumber = @paNumber, paBody = @paBody, paEdare = @paEdare WHERE (paId = @paId)">
                <DeleteParameters>
